...
async def button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "setkey":
        ...
        await query.message.reply_text(
            f"✅ O'lcham: *{ratio}*\n\n"
            "🎨 Endi rasmingizni yuboring — "
            "AI uni shu uslubda qayta yaratadi!",
            parse_mode="Markdown"
        )
        ...
        try:
            ...
            if r.status_code == 200:
                ...
                await query.message.reply_text(
                    "❌ Kredit ma'lumotini olishda xatolik!"
                )
...
async def admin_panel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for uid, data in users.items():
        ...
        text += (
            f"{status} *{data['name']}* | `{uid}` | "
            f"📸{data.get('upscale',0)} "
            f"🎨{data.get('generate',0)} | Key:{key}\n"
        )
...
async def unblock_user(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if uid in users:
        ...
        await update.message.reply_text(
            f"✅ {users[uid]['name']} blokdan chiqarildi!"
        )
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if response.status_code == 200:
        ...
        caption = (
            "✅ *AI Generate tayyor!* _(3 kredit)_"
            if mode == "generate"
            else "✅ *Upscale tayyor!* _(1 kredit)_"
        )
